...
(defun tddsg/config-packages ()
  ...
  (defun notify-output (type program output &optional reset-flags)
    (when reset-flags
  ...
  (defun notify-message (orig-fun &rest args)
    (let ((output (apply orig-fun args)))
      (cond
       ...
       ((check-sub-string "You should run LaTeX again" output)
       ...
       ;; magit
       ((check-sub-string "Running git push" output)
        (setq notify-git-push t))
       ((check-sub-string "Running git pull" output)
        (setq notify-git-pull t))
       ((check-sub-string "Git finished" output)
        (when notify-git-push (notify-output 'success "Git Push" output t))
        (when notify-git-pull (notify-output 'success "Git Pull" output t)))
       ((check-sub-string "Hit $ to see buffer magit-process" output)
        (when notify-git-push (notify-output 'error "Git Push" output t))
        (when notify-git-pull (notify-output 'error "Git Pull" output t)))
       ;; Compilation
...
(defun tddsg/config-keys ()
  ...
  (with-eval-after-load 'pdf-view
    ...
    (define-key pdf-view-mode-map (kbd "C-<mouse-1>")
      'pdf-sync-backward-search-mouse)
    ;; (define-key pdf-view-mode-map (kbd "<mouse-1>")
    ;;   'pdf-view-scroll-down-or-previous-page)
    ;; (define-key pdf-view-mode-map (kbd "<mouse-3>")
