...
(defcustom golden-ratio-auto-balance-other-window nil
  "Auto balance all other windows."
  :group 'golden-ratio
  :type 'boolean)

(defcustom golden-ratio-resize-lazy t
  "Do not resize the current window if it is already large enough."
  :group 'golden-ratio
  :type 'boolean)
...
(defun golden-ratio--resize-window (dimensions &optional window)
  (with-selected-window (or window (selected-window))
    (let ((nrow  (floor (- (first  dimensions) (window-height))))
      ...
      (unless (and golden-ratio-resize-lazy (< nrow 0))
        (when (window-resizable-p (selected-window) nrow)
          (enlarge-window nrow)))
      (unless (and golden-ratio-resize-lazy (< ncol 0))
...
(defun golden-ratio-toggle-auto-balance ()
  "Toggle balance other windows in golden ratio mode."
  (interactive)
  (if golden-ratio-auto-balance-other-window
      (setq golden-ratio-auto-balance-other-window nil)
    (setq golden-ratio-auto-balance-other-window t)))
...
(defun golden-ratio (&optional arg)
  ...
  (unless (or (and (not golden-ratio-mode) (null arg))
              ...
              (member (buffer-name) golden-ratio-exclude-buffer-names)
    ...
    (let ((dims (golden-ratio--dimensions))
      ...
      (when golden-ratio-auto-balance-other-window (balance-windows))
      (golden-ratio--resize-window dims)
      (when golden-ratio-recenter (scroll-right) (recenter)))))
